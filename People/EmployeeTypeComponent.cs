...
{
    public class EmployeeTypeComponent:ComponentBase
    {
        [Inject]
        ...
        private IEnumerable<EmployeeType> _getEmployeeType { get; set; }
        protected IEnumerable<EmployeeType> getEmployeeType
        {
        ...
        protected AddEmployeeType addComponent { get; set; }
        protected int ID;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var HRDResult = await Service.EmployeeTypes.GetAsync(PageIndex, PageSize);
            getEmployeeType = HRDResult;
            PageCount = await Service.EmployeeTypes.TotalPage(PageSize);
        ...
        public async System.Threading.Tasks.Task AddData(EmployeeType data)
        {
            try
            {
                if (data != null)
                {
                    await Service.EmployeeTypes.UpInsertAsync(data);
                }
        ...
        protected async Task ConfirmDelete_Click(bool deleteConfirmed)
        {
            if (deleteConfirmed)
            {
                var Result = await Service.EmployeeTypes.DeleteAsync(ID);
                await Load();
                ID = 0;
        ...
        {
            var HRDResult = await Service.EmployeeTypes.GetAsync(searchTerm);
            getEmployeeType = HRDResult;
