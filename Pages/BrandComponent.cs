...
{
    public class BrandComponent:ComponentBase
    {
        [Inject]
        ...
        private IEnumerable<Brand> _geCategory { get; set; }
        protected IEnumerable<Brand> Categories
        {
        ...
        protected AddBrand addCategory { get; set; }
        protected int ID;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var HRDResult = await Service.Brands.GetAsync(PageIndex, PageSize);
            Categories = HRDResult;
            PageCount = await Service.Brands.TotalPage(PageSize);
        ...
        public async System.Threading.Tasks.Task AddData(Brand data)
        {
            try
            {
                if (data != null)
                {
                    await Service.Brands.UpInsertAsync(data);
                }
        ...
        protected async Task ConfirmDelete_Click(bool deleteConfirmed)
        {
            if (deleteConfirmed)
            {
                var Result = await Service.Brands.DeleteAsync(ID);
                await Load();
                ID = 0;
        ...
        {
            var HRDResult = await Service.Brands.GetAsync(searchTerm);
            Categories = HRDResult;
