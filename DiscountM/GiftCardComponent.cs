...
{
    public class GiftCardComponent: ComponentBase
    {
        [Inject]
        ...
        private IEnumerable<GiftCard> _getRegion { get; set; }
        protected IEnumerable<GiftCard> getRegion
        {
        ...
        protected AddGiftCard addComponent { get; set; }
        protected int ID;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var HRDResult1 = await Service.GiftCards.GetAsync(PageIndex, PageSize);
            getRegion = HRDResult1;
            PageCount = await Service.GiftCards.TotalPage(PageSize);
        ...
        public async System.Threading.Tasks.Task AddData(GiftCard data)
        {
            try
            {
                if (data != null)
                {
                    await Service.GiftCards.UpInsertAsync(data);
                }
        ...
        protected async Task ConfirmDelete_Click(bool deleteConfirmed)
        {
            if (deleteConfirmed)
            {
                var Result = await Service.GiftCards.DeleteAsync(ID);
                await Load();
                ID = 0;
        ...
        {
            var HRDResult = await Service.GiftCards.GetAsync(searchTerm);
            getRegion = HRDResult;
