...
{
    public class DiscountComponent:ComponentBase
    {
        ...
        private IEnumerable<Discount> _getRegion { get; set; }
        protected IEnumerable<Discount> getRegion
        {
        ...
        protected AddDiscount addComponent { get; set; }
        protected int ID;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var HRDResult1 = await Service.Discounts.GetAsync(PageIndex, PageSize);
            getRegion = HRDResult1;
            PageCount = await Service.Discounts.TotalPage(PageSize);
        ...
        public async System.Threading.Tasks.Task AddData(Discount data)
        {
            try
            {
                if (data != null)
                {
                    await Service.Discounts.UpInsertAsync(data);
                }
        ...
        protected async Task ConfirmDelete_Click(bool deleteConfirmed)
        {
            if (deleteConfirmed)
            {
                var Result = await Service.Discounts.DeleteAsync(ID);
                await Load();
                ID = 0;
        ...
        {
            var HRDResult = await Service.Discounts.GetAsync(searchTerm);
            getRegion = HRDResult;
